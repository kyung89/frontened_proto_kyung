  ...
  const handleClick = async (e) => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        // 사용자 정보를 로컬 스토리지에 저장
        handleLogin(username); // 로그인 성공 후 처리: Login 으로 넘어감(handleLogin 함수)
      } else {
      ...
      <div className="max-w-sm mx-auto space-y-4">
        ...
        <form onSubmit={handleClick}>
          <div>
            <label
              htmlFor="username"
              className="block py-1 text-lg font-neucha font-bold"
            >
              username
            </label>
            <input
              ...
              type="text"
              id="usernameOrEmail"
              className="border w-full py-2 px-2 rounded shadow hover:border-green-600 ring-1 ring-inset ring-gray-300 font-mono"
          ...
          <div>
            <label
              htmlFor="pw"
              className="block py-1 font-neucha font-bold text-lg"
            >
              password
            </label>
            <input
          ...
          <div className="flex gap-3 pt-3 items-center">
            <button
              type="submit"
              className="border hover:border-green-600 px-4 py-2 font-semibold rounded-lg shadow ring-1 ring-inset ring-gray-300 font-neucha text-lg"
            >
              sign in
            </button>
            <a href="#" className="font-neucha font-bold text-base">
              forget password?
            </a>
          </div>
        </form>
        <div className="flex items-center justify-center  font-serif text-zinc-700 font-neucha font-bold text-lg">
          <p> -others- </p>
        </div>
        ...
        <div className="flex gap-2 items-center justify-center">
          <Link to="/join">
            <button className="border hover:border-green-600 mt-3 px-4 py-2 font-semibold rounded-lg shadow ring-1 ring-inset ring-gray-300 font-neucha text-lg">
              SignUp
            </button>
          </Link>
        </div>
